/*
 * FastNoise.js
 * High-performance Simplex Noise implementation
 * 
 * Based on public domain Simplex Noise implementations
 * Optimized for V8/Node.js by using pre-computed permutation tables
 * and minimizing object allocations.
 */

// Permutation table (doubled for overflow performance)
...
function seed(seedVal) {
    // Simple LCG to shuffle table based on seed
 ...
 * @returns {number} Noise value (roughly -1 to 1, can exceed)
 */
function fractal(x, z, octaves, persistence = 0.5, lacunarity = 2.0) {
    let total = 0;
    let frequency = 1;
    let amplitude = 1;
    ...
    // Normalize to roughly -1 to 1 range (not strictly clamped)
...
/**
 * Rigid/Sharp Noise
 * Uses absolute value for sharp creases (valleys/ridges)
 */
function rigid(x, z, octaves = 4) {
    let total = 0;
    let frequency = 1;
    let amplitude = 1;
    let maxValue = 0;
    ...
    for (let i = 0; i < octaves; i++) {
        // |noise| creates creases at 0
        // 1 - |noise| creates peaks at 0
